...
draw.pairwise.venn <- function(
    ...
    if (cross.area != 0 & (cross.area == area2 | cross.area == area1)) {
    ...
    wrapLab <- function(num) {
        ...
        if (print.mode[1] == 'percent') {
            stri <- paste0(signif(num * 100 / denom, digits = sigdigs), '%');
        ...
        if (print.mode[1] == 'raw') {
            ...
            if (isTRUE(print.mode[2] == 'percent')) {
                stri <- paste0(stri, '\n(', signif(num * 100 / denom, digits = sigdigs), '%)');
                }
            }
...
#	flog.info(c(area1,area2,cross.area),name="VennDiagramLogger");
    ...
    if (scaled & !special.inclusion & !special.exclusion & !special.coincidental) {
        ...
        if (length(cex.prop) > 0) {
                ...
                if (cex.prop == 'lin') {
                    func <- function(x) x
                        ...
                        func <- log10;
                            ...
                            flog.error(paste0('Unknown value passed to cex.prop: ', cex.prop), name = 'VennDiagramLogger');
            ...
            for (i in 1:length(areas)) {
                ...
                if (cex[i] <= 0) {
                    stop(paste0('Error in rescaling of area labels: the label of area ', i, ' is less than or equal to zero'));
                    }
                }
            }
        ...
        if (cat.default.pos == 'outer') {
            ...
            cat.pos.2 <- find.cat.pos(0.5 - offset * (r1 - r2), 0.5, cat.pos[2], cat.dist[2], r2);
                ...
                cat.pos.2 <- find.cat.pos(area.2.pos, 0.5, cat.pos[2], cat.dist[2]);
                    ...
                    raise.error('Invalid value for "cat.default.pos", should be either "outer" or "text"');
                    }
    ...
    # rotate Venn if necessary and add other adjustments to plot

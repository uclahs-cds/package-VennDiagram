...
draw.sp.case <- function(
    ...
    if (print.mode[1] == 'percent') {
        processedLabels <- paste0(signif(area.list / sum(area.list) * 100, digits = sigdigs), '%');
    ...
    if (print.mode[1] == 'raw') {
        ...
        if (isTRUE(print.mode[2] == 'percent')) {
            processedLabels <- paste0(processedLabels, '\n(', signif(area.list / sum(area.list) * 100 , digits = sigdigs), '%)');
            }
        }
        ...
        if ('outer' == cat.default.pos) {
            this.cat.pos <- VennDiagram::find.cat.pos(
                ...
                this.cat.pos <- VennDiagram::find.cat.pos(
                    ...
                    raise.error('Invalid cat.default.pos setting');
                    }
